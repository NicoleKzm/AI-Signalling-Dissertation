...
# ── Controls ───────────────────────────────────────────────────────
# Firm size proxy. Sector membership is time-invariant within each firm
# and is therefore absorbed by the firm fixed effects; no sector dummies
# are included.
...
for hypothesis, dv in dependent_vars.items():
    ...
    try:
        ...
        # 95% confidence interval (linearmodels, t-distribution)
        ci = result.conf_int().loc[independent_var]
...
model_df = df[['Revenue_Growth_%', 'mean_signal_score', 'log_revenue']].dropna()

exog = model_df[['mean_signal_score', 'log_revenue']]
...
for hypothesis, dv in dependent_vars.items():
    print(f"\n── {hypothesis}: DV = {dv} ──")
    model_df = df[[dv, 'is_transitional', 'is_substantive', 'log_revenue']].dropna()

    exog = sm.add_constant(
        model_df[['is_transitional', 'is_substantive', 'log_revenue']]
    )
...
for check_name, check_df in checks.items():
    ...
    for hypothesis, dv in dependent_vars.items():
        ...
        if len(model_df) < 15:
            ...
            continue
        exog = sm.add_constant(model_df[[independent_var] + controls])
        try:
            model = PanelOLS(
                model_df[dv],
                exog,
