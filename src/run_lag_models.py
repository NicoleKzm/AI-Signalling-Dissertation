...
DATA_RESULTS = ROOT / "results"
DATA_RESULTS.mkdir(parents=True, exist_ok=True)

df = pd.read_csv(DATA_PROCESSED / "panel_dataset.csv")
...
for label, dv, iv in MODELS:
    ...
    rows.append({
        "Model": label, "DV": dv, "IV": iv,
        "beta": round(b, 3), "SE": round(se, 3), "p_value": round(p, 4),
        "CI_lo": round(ci["lower"], 2), "CI_hi": round(ci["upper"], 2),
        "N": int(res.nobs), "R2_within": round(res.rsquared_within, 4),
    })
    print(f"{label}\n  beta={b:.3f}, SE={se:.3f}, p={p:.4f}, CI=[{ci['lower']:.2f}, {ci['upper']:.2f}], N={int(res.nobs)}")
