...
# ══════════════════════════════════════════════════════════════════
base_fy = pd.read_csv('signalling_scores.csv')[['firm', 'year', 'total_passages', 'mean_signal_score']].copy()
base_fy = base_fy.rename(columns={'mean_signal_score': 'mean_signal_score_baseline'})

classifications = pd.read_csv('all_classifications.csv')
...
results_df.to_csv('aggregation_sensitivity.csv', index=False)
print(f"\naggregation_sensitivity.csv saved -- {len(results_df)} rows")

tost_df.to_csv('aggregation_sensitivity_h3_tost.csv', index=False)
print(f"aggregation_sensitivity_h3_tost.csv saved -- {len(tost_df)} rows")
