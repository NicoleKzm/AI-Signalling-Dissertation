...
log(f"Source: data/panel_dataset.csv -- {len(panel)} rows, {panel['firm'].nunique()} firms")
...
reg_results = pd.read_csv('results/regression_results.csv')
...
df_full = pd.read_csv('data/panel_dataset.csv')
...
with open('results/verification_results.txt', 'w') as f:
    f.write("\n".join(OUT) + "\n")
print("\nresults/verification_results.txt saved.")
