...
df.to_csv(out / "disclosure_depth_dataset.csv", index=False)
...
res.to_csv(out / "depth_models_summary.csv", index=False)

print("DONE")
print(sorted([p.name for p in out.iterdir() if p.name.startswith("depth") or p.name == "disclosure_depth_dataset.csv"]))
